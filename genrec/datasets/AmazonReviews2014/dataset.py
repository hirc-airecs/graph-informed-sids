...
class AmazonReviews2014(AbstractDataset):
    ...
    def _load_reviews(self, path: str) -> list:
        ...
        for inter in self._parse_gz(path):
            ...
            reviews.append((user, item, int(time), int(rating)))
        return reviews
    ...
    def _get_item_seqs(self, reviews: list[tuple]) -> dict:
        ...
        self.log("Grouping interactions by user")
        seqs = {}
        for data in tqdm(reviews):
            user, item, time, rating = data
            seqs[user] = seqs.get(user, []) + [(item, time, rating)]

        # Sort items by time
        self.log("Sorting user sequences by time")
        for user, item_time_rating in tqdm(seqs.items()):
            item_time_rating.sort(key=lambda x: x[1])
            seqs[user] = {
                "item_seq": [_[0] for _ in item_time_rating],
                "time_seq": [_[1] for _ in item_time_rating],
                "rating_seq": [_[2] for _ in item_time_rating],
            }
        return seqs
    ...
    def _process_reviews(self,
        ...
        Returns:
            all_seqs (dict): A dictionary containing the user-item sequences.
            id_mapping (dict): A dictionary containing data maps.
        """
