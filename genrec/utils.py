...
def get_file_name(config: dict, suffix: str = ""):
    ...
    hash_src = f"{config_str}|{command_line_args}"
    md5 = hashlib.md5(hash_src.encode("utf-8")).hexdigest()[:8]

    run_id = _slug(config["run_id"], max_len=40)
    run_time = _slug(config["run_local_time"], max_len=32)
...
def get_trainer(model_name: Union[str, AbstractModel]):
    ...
    """
    module_name = f'genrec.models.{model_name}.trainer'
    sys.modules.pop(module_name, None)
    importlib.invalidate_caches()
...
def get_pipeline(model_name: Union[str, AbstractModel]):
    ...
    """
    module_name = f'genrec.models.{model_name}.pipeline'
    sys.modules.pop(module_name, None)
    importlib.invalidate_caches()
...
def deep_update(config: dict, updates: dict, delete_keyword: str = "__DELETE__"):
    for k, v in updates.items():
        if isinstance(v, dict) and k in config and isinstance(config[k], dict):
            deep_update(config[k], v)
        else:
            if v == delete_keyword:
                config.pop(k)
            else:
...
def _to_tensorboard_hparam_value(value):
...
def config_for_log(config: dict) -> dict:
    config = {
        str(key): _to_tensorboard_hparam_value(value)
    ...
    for k in keys:
        ...
        if isinstance(v, list):
            ...
            for flatten_k, flatten_v in flatten_dict(v, root_str=k).items():
                config[flatten_k] = str(flatten_v)
            config.pop(k, None)
    return config


def flatten_dict(d: dict, root_str: str = "") -> dict:
    flatten_d = {}
    for k, v in d.items():
        if root_str:
            k = f"{root_str}.{k}"
        if isinstance(v, dict):
            flatten_d |= flatten_dict(v, k)
        else:
            flatten_d[k] = v
    return flatten_d
