...
import random
from gensid.models.layers import kmeans, sinkhorn_algorithm


class VectorQuantizer(nn.Module):
    def __init__(self, n_e, e_dim, mu=0.25, beta=1, kmeans_init=False, kmeans_iters=10, sk_epsilon=0.01, sk_iters=100):
        super().__init__()
        self.n_e = n_e
        self.e_dim = e_dim
        self.beta = beta
    ...
    def constrained_km(self, data, n_clusters=10):
        ...
        size_min = min(len(data) // (n_clusters * 2), 1024)  # 50 for the very first time, 10 the latter
    ...
    def forward(self, x, label, idx, use_sk=True):
        ...
        return x_q, loss, indices, x_q_codebook
