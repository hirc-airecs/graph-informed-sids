...
METHOD_NAME="LETTER_t5"
EMB_MODEL="sentence-t5-base"
# METHOD_NAME="LETTER_qwen"
...
#
# Available presets:
#   rqvae_args_full       - all flags enabled (default)
...
#   rqvae_args_minimal    - ablation: no graph signal, no edge reconstruction
# ============================================================
...
prepare_cf_embeddings() {
  local DATASET="$1"
  local SEED="$2"
  local ALIAS="${DATASET}_seed${SEED}_pretrainCF_SASRec"
  ...
  should_skip "pretrain_cf" || run_step "Run SASRec | [${ALIAS}]" bash -c "
    ...
    should_skip "copy_cf" || run_step "Copy CF embeddings [${ALIAS}]" \
      python map_cf_embeddings.py --dataset-id="${DATASET}" --log-path="${LOG_FILE}"
...
for DATASET in "${DATASETS[@]}"; do
    # Prepare CF embeddings once per dataset (on the first seed iteration)
    _sem_wait
    prepare_cf_embeddings "${DATASET}" "${SEEDS[0]}"
done
